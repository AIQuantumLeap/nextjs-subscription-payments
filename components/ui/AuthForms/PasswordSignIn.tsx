  ...
  return (
    <div className="space-y-4">
      <form noValidate onSubmit={handleSubmit} className="space-y-3">
        ...
        >
          {isSubmitting ? <><LoadingDots /> Signing in…</> : 'Log In'}
        </button>
      </form>

      {/* Forgot */}
      <div className="text-center">
        <Link href="/signin/forgot_password" className="text-sm text-zinc-400 hover:text-zinc-200 transition-colors">
          Forgot password?
        </Link>
      </div>
      ...
      <p className="text-center text-sm text-zinc-500 pt-2 border-t border-zinc-800">
        ...
        <Link href="/signin/signup" className="text-zinc-200 font-medium hover:text-white transition-colors underline underline-offset-2">
          Create Your Account
        </Link>
      </p>
    </div>
